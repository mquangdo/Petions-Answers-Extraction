...
from common import find_entities, postprocess_noi_dung, postprocess_tra_loi, _clean_text, _extract_signer, _extract_llm
...
# ---------------------------------------------------------------------------
# Biến thể ngắn của mục trả lời: "Kết quả trả lời kiến nghị" (vd: Nghệ An 7330).
# Yêu cầu heading markdown + số ở đầu dòng để không bắt nhầm câu văn thông thường.
# ---------------------------------------------------------------------------
_S2_SHORT_RE = re.compile(
    r"^\s*[#*_>\s]*\d*\.?\s*[#*_>\s]*k[ÊÉếe]t\s+qu[ảa]\s+trả\s+lời\s+kiến\s+ngh[ịi]",
    re.I,
)

# ---------------------------------------------------------------------------
# Biến thể S2 dạng "Nội dung kiến nghị đã được giải quyết" (vd: Đà Nẵng 8045).
# Yêu cầu heading markdown + số đầu dòng để không bắt nhầm heading con dạng
# "a) Nội dung kiến nghị đã được giải quyết" nằm bên trong phần trả lời.
# ---------------------------------------------------------------------------
_S2_ALT_RE = re.compile(
    r"^\s*[#*_>\s]*\d+\.\s*[#*_>\s]*Nội dung\s+kiến\s+ngh[ịi]\s+đã\s+được\s+giải\s+quyết",
    re.I,
)
...
# ---------------------------------------------------------------------------
# Regex nhận diện tiêu đề mục "Nội dung kiến nghị" (S1) theo quy tắc:
#   - "Nội dung kiến nghị của cử tri ( Kiến nghị số 120 )"
#   - "Nội dung kiến nghị số 117 của cử tri tỉnh An Giang"
#   - "Nội dung kiến nghị của cử tri: Cử tri tỉnh Đồng Nai ..."
# Chỉ khớp heading markdown (#, `**`) + số tùy chọn, và KHÔNG khớp các heading
# con dạng "2.1./2.2. Nội dung kiến nghị về ..." nằm bên trong phần trả lời
# (chúng không theo sau bởi "của cử tri"/"số ...").
# ---------------------------------------------------------------------------
_S1_RE = re.compile(
    r"^\s*[#*_>\s]*\d*(?:\.\d+)*\.?\s*[#*_>\s]*Nội dung\s+kiến\s+ngh[ịi]\s+(?:của\s+cử\s+tr[ìi]|số)",
    re.I,
)
...
# ---------------------------------------------------------------------------
# (Format 3) Mốc mở đầu block TRẢ LỜI.
# Bản nnmt chỉ khớp heading có "Về/Đối với kiến nghị số X". Bộ Nội vụ (nv) thường
# KHÔNG ghi số kiến nghị mà chỉ đánh số mục đáp án, ví dụ:
#   "2.1. Về việc sắp xếp thôn, tổ dân phố"
#   "**2.2.** Về cơ chế bố trí, sử dụng ..."
#   "2.1. Về kiến nghị việc ban hành chính sách ..." (có chữ "kiến nghị", không số)
# Vì vậy khớp MỌI heading cấp "2.x." và capture toàn bộ phần chữ còn lại để:
#   - lấy TẬP số nếu heading có "kiến nghị số" (findall \d+);
#   - nếu không có số -> map theo THỨ TỰ block (item k <-> block k).
# (?!\d) sau "2.x." để loại heading cấp con "2.x.y." (vd "2.1.1. ...").
# ---------------------------------------------------------------------------
...
_SO_CONG_VAN_RE = re.compile(
    r"^\s*[#*_>\s]*\*{0,2}Số\*{0,2}\s*[:\-]?\s*\*{0,2}\s*([\d]*\s*/[A-Za-z0-9Đđ\-\&()_.]+(?:\s*[A-Za-z0-9Đđ\-\&()_.]+)*)\*{0,2}(?=\s*V/v|\s*$)",
    re.I | re.M,
)
